...
Character::Character(char c, int index, int column, int line, char* source) :
cargo(c),
sourceIndex(index),
...
void Character::GetCargo(char* c) const { *c = cargo; }
int Character::GetColumnIndex() const	{ return sourceColumnIndex; }
int Character::GetLineIndex() const		{ return sourceLineIndex; }
